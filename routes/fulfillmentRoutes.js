    ...
    app.post('/', async (req, res) => {
        ...
        async function search(agent) {
            ...
            try{
                ...
                agent.add(`Here are the items I found using the keyword(s) "${item}." If you'd like me to search for anything else just type "product" or "search."`);
                ...
                if(modifiedItems){
                    let cards = modifiedItems.map(item => formatCardItem(item.title, item.image, (item.price + 1), item.source, item.link));
